...
(defn to-points [input]
  (->> (clojure.string/split input #",")
       (map (fn [s]
               (let [[_ dir n] (re-find #"(\w)(\d+)" s)]
                 [dir (Integer/parseInt n)])))
       (mapcat #(take (second %) (repeat (get shifts (first %)))))
       (reductions (fn [[x y] [dx dy]] [(+ x dx) (+ y dy)]) [0 0])))

;; part1 
(->> (intersection (set (to-points input1))
                   (set (to-points input2)))
...
;; part2
(let [wire1 (to-points input1)
      wire2 (to-points input2)]
